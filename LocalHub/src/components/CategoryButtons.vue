<script setup>
import { ref, watch, computed } from 'vue'

const props = defineProps({
  categories: { type: Array, default: () => ['관광지', '레포츠', '문화시설', '쇼핑', '숙박', '여행코스', '축제공연행사'] },
  selected: { type: String, default: '' }
})
const emit = defineEmits(['select'])
const current = ref(props.selected)

watch(() => props.selected, v => (current.value = v))

const icons = {
  관광지: '📍',
  레포츠: '🏄‍♂️',
  문화시설: '🏛️',
  쇼핑: '🛍️',
  숙박: '🏨',
  여행코스: '🗺️',
  축제공연행사: '🎉'
}

function onClick(cat) {
  current.value = cat
  emit('select', cat)
}

const items = computed(() => props.categories.map(c => ({
  label: c,
  icon: icons[c] ?? '🔖'
})))
</script>

<template>
  <section class="category-section">
    <h3 class="category-heading">카테고리 바로가기</h3>

    <div class="category-grid" role="list">
      <button v-for="it in items" :key="it.label" class="card" :class="{ active: it.label === current }"
        @click="onClick(it.label)" role="listitem" :aria-pressed="it.label === current">
        <div class="icon">{{ it.icon }}</div>
        <div class="label">{{ it.label }}</div>
      </button>
    </div>
  </section>
</template>

<style scoped>
.category-section {
  max-width: 1400px;
  margin: 24px auto 12px;
  padding: 0 24px;
  box-sizing: border-box;
}

/* 제목(h3) 스타일 — bold */
.category-heading {
  font-size: 20px;
  font-weight: 800;
  color: #0f172a;
  margin: 0 0 12px 8px;
}

/* 넓이 중심: 카드 폭을 키우기 위해 컨테이너 최대너비 증가하고 칸 최소값 설정 */
.category-grid {
  max-width: 1600px;
  margin: 0 auto;
  padding: 0 8px 18px;
  box-sizing: border-box;

  display: grid;
  grid-template-columns: repeat(4, minmax(260px, 1fr));
  grid-auto-rows: 220px;
  gap: 28px;
  align-items: stretch;
  justify-items: center;
}

/* 카드 스타일 */
.card {
  width: 100%;
  height: 100%;
  min-width: 260px;
  min-height: 200px;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  gap: 16px;
  padding: 20px;
  border-radius: 18px;
  border: 1px solid rgba(107, 120, 232, 0.08);
  background: #fff;
  box-shadow: 0 12px 30px rgba(11, 16, 40, 0.05);
  cursor: pointer;
  transition: transform .18s, box-shadow .18s, border-color .12s;
}

.card:hover {
  transform: translateY(-8px);
  box-shadow: 0 24px 60px rgba(11, 16, 40, 0.10);
  border-color: rgba(107, 120, 232, 0.16);
}

.icon {
  font-size: 64px;
}

.label {
  font-size: 18px;
  font-weight: 800;
}

@media (max-width: 1400px) {
  .category-grid {
    grid-template-columns: repeat(3, minmax(220px, 1fr));
    grid-auto-rows: 180px;
    max-width: 1200px;
  }

  .card {
    min-width: 220px;
    min-height: 180px;
    padding: 18px;
  }

  .icon {
    font-size: 48px;
  }

  .label {
    font-size: 16px;
  }
}

@media (max-width: 1000px) {
  .category-grid {
    grid-template-columns: repeat(2, minmax(200px, 1fr));
    grid-auto-rows: 160px;
    max-width: 900px;
  }

  .card {
    min-width: 200px;
    min-height: 160px;
    padding: 16px;
  }

  .icon {
    font-size: 40px;
  }

  .label {
    font-size: 15px;
  }
}

@media (max-width: 600px) {
  .category-grid {
    grid-template-columns: 1fr;
    grid-auto-rows: 140px;
    padding: 8px 10px;
    max-width: 480px;
  }

  .card {
    min-width: auto;
    min-height: 140px;
    padding: 12px;
  }

  .icon {
    font-size: 36px;
  }

  .label {
    font-size: 14px;
  }
}

/* active state */
.card.active {
  border-color: rgba(91, 69, 214, 0.18);
  box-shadow: 0 20px 40px rgba(91, 69, 214, 0.06);
}

/* focus/keyboard 접근성 */
.card:focus-visible {
  outline: 3px solid rgba(91, 69, 214, 0.12);
  outline-offset: 4px;
}
</style>